...
fun ListaUsuariosScreen() {
    ...
    // Estado con tipo explícito nullable
    var seleccionado by remember { mutableStateOf<Usuario?>(null) }
    // Si quieres que sobreviva a recomposiciones por configuración (rotación), usa:
    // var seleccionado by rememberSaveable(stateSaver = Saver(usuarioSaver)) { mutableStateOf<Usuario?>(null) }
        ...
        LazyColumn(
            ...
            items(items = usuarios, key = { it.id }) { user ->
                Card(
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable { seleccionado = user } // <- asignación correcta
                ) {
