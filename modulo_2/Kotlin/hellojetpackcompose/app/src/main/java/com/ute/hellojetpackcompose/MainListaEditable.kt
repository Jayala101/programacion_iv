...
fun ListaEditableScreen() {
    ...
    // Guarda solo el ID seleccionado para que sea saveable sin escribir un Saver personalizado
    var seleccionadoId by rememberSaveable { mutableStateOf<Int?>(null) }
    val seleccionado: UsuarioEditable? = remember(usuarios, seleccionadoId) {
        usuarios.firstOrNull { it.id == seleccionadoId }
    }
                ...
                Card(Modifier.fillMaxWidth()) {
                    Row(
                        ...
                        Column(
                            modifier = Modifier
                                .weight(1f)
                                .clickable { seleccionadoId = user.id }
                        ) {
                            Text(user.nombre, style = MaterialTheme.typography.titleMedium)
                            Text(user.rol, style = MaterialTheme.typography.bodyMedium)
                        }
                        TextButton(onClick = {
                            // elimina y limpia selección si corresponde
                            usuarios.removeAll { it.id == user.id }
                            if (seleccionadoId == user.id) seleccionadoId = null
                        }) { Text("Eliminar") }
                    }
                }
            }
